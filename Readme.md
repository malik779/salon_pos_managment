...
> **Important:** The Codespace/CI environment used for this repository does **not** provide the `dotnet` CLI. The backend source compiles in a normal .NET 8 SDK but you cannot run `dotnet build` inside the hosted environment. Use Docker-based workflows (below) or run the solution on your workstation.
...
| .NET 8 SDK | Build/run backend services outside containers |
...
- **dotnet CLI missing**: run services inside Docker until you install the .NET 8 SDK locally.
